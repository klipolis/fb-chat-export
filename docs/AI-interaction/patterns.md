...
- **Post-interaction saves record work to AI interaction docs**. After each session, add a new entry to `interaction-log.md` (exact user input, AI summary, commits), update relevant guidance and patterns, and keep `session-plan.md` current.
- **Post-close saves record session outcomes for continuity**. Update `interaction-log.md` with the final session entry (exact user input, AI summary, commit references). Update `session-plan.md` if the chronological plan is outdated.
