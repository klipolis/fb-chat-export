...
function testBrowserExportDomRegression() {
  ...
  assert.strictEqual(
    entry.dateText,
    '2026-04-17 15:45',
    'Date text should be normalized to ISO-friendly format'
  );
...
function testGoldenTxtSnapshots() {
  ...
  assert.ok(
    fs.existsSync(actualOffPath),
    'Content-off TXT export missing for golden snapshot test'
  );
...
function testRawHtmlRegression() {
  ...
  assert.strictEqual(
    voiceNode.data_preview.raw_meta.duration,
    '0:20',
    'voice-note raw_meta.duration should keep raw input value'
  );
  ...
  assert.strictEqual(
    videoNode.data_preview.raw_meta.duration,
    '31 mins',
    'video-call raw_meta.duration should keep raw input value'
  );
...
function testLinkFileUsesLinkType() {
  ...
  assert.strictEqual(
    result.type,
    'link',
    'link-text files should be classified as link when raw meta link is present'
  );
  assert.ok(
    result.text.startsWith('https://www.scan.co.uk/'),
    'link-text content should be prepended with the URL'
  );
  assert.ok(
    result.text.includes('image sent'),
    'link-text content should keep message text after the URL'
  );
  assert.ok(
    /\d+ chars$/.test(result.contentLength),
    'link-text previews with text should include content_length'
  );
...
function testMissedCallNoDuration() {
  ...
  assert.strictEqual(
    ...
    undefined,
    'missed-call previews should not include content_length'
  );
}
...
function testTextFileDoesNotBecomeLink() {
  ...
  assert.strictEqual(
    nodes[0].data_preview.content_type,
    'text',
    'text.html should be classified as text'
  );
  assert.notStrictEqual(
    nodes[0].data_preview.content,
    'link',
    'text.html content should not be reduced to link'
  );
}
...
function testImageFileIsImageType() {
  ...
  assert.strictEqual(
    nodes[0].data_preview.content_type,
    'image',
    'image.html should be classified as image'
  );
  assert.strictEqual(
    nodes[0].data_preview.content,
    'image sent',
    'image message preview should use image sent content'
  );
}
...
function testAnonymizeChatNames() {
  ...
  assert.ok(
    cleaned.includes('aria-label="At Wednesday 7:54pm, Alpha"'),
    'Sender aria-label should be anonymized'
  );
  assert.ok(
    cleaned.includes('aria-label="Enter, Message sent Wednesday 7:54pm by Alpha"'),
    'Enter message sender should be anonymized'
  );
  assert.ok(
    cleaned.includes('<img alt="Alpha Leon"'),
    'Profile alt text sender word should be anonymized'
  );
  assert.ok(
    !cleaned.includes('Rob deleted a message'),
    'Message content should be anonymized once the sender name is found'
  );
  assert.ok(
    !cleaned.includes('Message body mentioning Rob should be anonymized.'),
    'Message body text references should be anonymized once the sender name is found'
  );
  assert.ok(
    cleaned.includes('<title>Alpha</title>'),
    'Chat title should be anonymized if it contains the sender name'
  );
}
...
function testAnonymizeChatNamesPreservesYou() {
  ...
  const cleaned = anonymizeChatNames(rawHtml);
  assert.ok(
    cleaned.includes('aria-label="At Thursday 5:34pm, You"'),
    'Self-label You should remain unchanged'
  );
  assert.ok(
    cleaned.includes('aria-label="Enter, Message sent Thursday 5:34pm by You"'),
    'Self-label by You should remain unchanged'
  );
}
...
function testAnonymizeChatNamesOnlyReplacesShortSenderNames() {
  ...
  const cleaned = anonymizeChatNames(rawHtml);
  assert.ok(
    cleaned.includes('Alpha deleted a message'),
    'Valid short sender name should be anonymized'
  );
  assert.ok(
    cleaned.includes('All of this is a long phrase that is not a sender name'),
    'Long non-name phrases should not be anonymized'
  );
}
...
function testBuildServerTextExport() {
  const serverBuild = require('child_process').spawnSync('node', ['src/build-server.js'], {
    encoding: 'utf8',
  });
  assert.strictEqual(
    serverBuild.status,
    0,
    `build-server failed: ${serverBuild.stderr || serverBuild.stdout}`
  );
  const txtDir = path.join(__dirname, '..', 'demo/output-txt');
  assert.ok(fs.existsSync(txtDir), 'demo/output-txt not created');
  ...
  assert.ok(
    !fs.existsSync(path.join(txtDir, 'fb-chats-export-summary.json')),
    'Summary JSON export should not be generated'
  );
  ...
  assert.ok(
    ...
    'Summary TXT should use posts for total summary'
  );
  assert.ok(
    /\nAlpha Summary\n\d+\s+post(?:s)?\s*\/\s*\d+\s+days\n/.test(summaryTxt),
    'Summary TXT should use post/posts for Alpha Summary'
  );
  assert.ok(
    /\nYoughurt Summary\n\d+\s+post(?:s)?\s*\/\s*\d+\s+days\n/.test(summaryTxt),
    'Summary TXT should use post/posts for Youghurt Summary'
  );
  ...
  assert.ok(
    summaryTxt.includes(` / ${uniqueDays} days`),
    'Summary TXT total days must reflect unique days across all messages'
  );

  assert.ok(
    contentOn.includes('Method: server'),
    'Content-on export should include the server method header'
  );
  assert.ok(
    contentOff.includes('Method: server'),
    'Content-off export should include the server method header'
  );
  assert.ok(
    contentOn.includes('Message types:'),
    'Content-on export should include a message types header'
  );
  assert.ok(
    contentOff.includes('Message types:'),
    'Content-off export should include a message types header'
  );
  ...
  assert.ok(
    ...
    'Content-on summary should include a total count line without prefix'
  );
  assert.ok(
    /\n~\s+\d+\s+text;\n/.test(contentOn),
    'Content-on summary should include rough text totals as ~ list items'
  );
  assert.ok(
    /\n~\s+\d+\s+images\n/.test(contentOn),
    'Content-on summary should include rough image totals as ~ list items'
  );
  assert.ok(
    /\n~\s+\d+\s+calls\s+\d+\s+mins\n/.test(contentOn),
    'Content-on summary should include rough call totals without brackets'
  );
  assert.ok(
    /\nAlpha Summary\n/.test(contentOn),
    'Content-on summary should include Alpha Summary section'
  );
  assert.ok(
    /\nYoughurt Summary\n/.test(contentOn),
    'Content-on summary should include Youghurt Summary section'
  );
  assert.ok(
    /\nAlpha Summary\n\d+\s+(?:message|messages)\s*\/\s*\d+\s+(?:day|days)\n~\s+\d+\s+text;\n~\s+\d+\s+images\n~\s+\d+\s+calls\s+\d+\s+mins\n/.test(
      contentOn
  ...
  rawFiles.forEach((fileName) => {
    ...
    assert.ok(
      contentOn.includes(`- ${sample}`),
      `Content-on header should list ${fileName} as dashed item`
    );
    assert.ok(
      contentOff.includes(`- ${sample}`),
      `Content-off header should list ${fileName} as dashed item`
    );
  });

  const bodyStartOn = contentOn.lastIndexOf('\n---\n');
  const bodyAfterSummary =
    bodyStartOn > -1 ? contentOn.slice(bodyStartOn + '\n---\n'.length) : contentOn;
  ...
  assert.strictEqual(
    bodyLinesOn.length,
    rawFiles.length,
    'Content-on export should contain one message per raw file'
  );
  assert.strictEqual(
    bodyLinesOff.length,
    rawFiles.length,
    'Content-off export should contain one message per raw file'
  );
  ...
  assert.ok(
    summaryTxt.includes(` / ${allMessageDayCount} `),
    'Summary TXT should include total day count based on all messages'
  );

  const basePattern = /^\[\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}\]\s[^:]+:\s[^/]+(?:\s\/\s.*)?$/;
  assert.ok(
    bodyLinesOn.every((line) => basePattern.test(line)),
    'Each content-on line should match [date time] name: type duration length / content format'
  );
  assert.ok(
    bodyLinesOff.every((line) => basePattern.test(line)),
    'Each content-off line should match [date time] name: type duration length format'
  );

  rawFiles.forEach((fileName) => {
    const typeName = path.parse(fileName).name;
    assert.ok(
      bodyLinesOn.some((line) => line.includes(` ${typeName}`)),
      `Content-on body should include one line for ${typeName}`
    );
    assert.ok(
      bodyLinesOff.some((line) => line.includes(` ${typeName}`)),
      `Content-off body should include one line for ${typeName}`
    );
  });

  assert.ok(
    bodyLinesOn.some((line) => line.includes('video-call 31:00 mins')),
    'Video call lines should include duration in canonical format'
  );
  assert.ok(
    bodyLinesOn.some((line) => /\blink-text\b(?:\s+\d+ chars)?\s*\/\s*https?:\/\//i.test(line)),
    'link-text line should include URL content in content-on export'
  );
  assert.ok(
    bodyLinesOn.some((line) => /\blink-embed-no-text\b\s*\/\s*https?:\/\//i.test(line)),
    'link-embed-no-text line should include URL content in content-on export'
  );
  assert.ok(
    bodyLinesOn.some((line) => /\blink-text\b\s+\d+ chars\s*\/\s*https?:\/\//i.test(line)),
    'link-text lines with text should include content length in content-on export'
  );
  assert.ok(
    bodyLinesOff.every((line) => !line.includes(' / ')),
    'Content-off export should not include slash-delimited content'
  );
  const offTextLengthLine = bodyLinesOff.find((line) => /\btext\b\s+\d+ chars(?:\s|$)/i.test(line));
  assert.ok(
    offTextLengthLine,
    'Content-off export should still include text content length when content is disabled'
  );
  ...
  assert.ok(
    contentOn.includes('Alpha'),
    'Content-on export should contain anonymized sender names as Alpha'
  );
  assert.ok(
    contentOff.includes('Alpha'),
    'Content-off export should contain anonymized sender names as Alpha'
  );
  assert.ok(
    !contentOn.includes('Rob'),
    'Content-on export should not contain raw sender names like Rob'
  );
  assert.ok(
    !contentOff.includes('Rob'),
    'Content-off export should not contain raw sender names like Rob'
  );
}
