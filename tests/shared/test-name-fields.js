...
tap.test('data_raw.name and data_preview.name fields with alias mapping', (t) => {
  // Test Csaba/Sztoika from voice-note-2.html (should map to XYZ per any→XYZ mapping)
  ...
  t.equal(voiceNoteNode.data_raw.name, 'Csaba', 'data_raw.name should preserve original sender name');
  
  // Check data_preview.name receives aliased name (Csaba → XYZ per any→XYZ mapping)
